...
function EmergencyApp() {
    ...
    return (<div className="relative h-screen w-full bg-gray-50 font-sans overflow-hidden">
      {/* Status Bar */}
      ...
      <div className="h-full pb-16">
        {currentScreen !== 'home' && (<div className="bg-white h-12 flex items-center px-4 shadow-sm">
            <button onClick={function () { return setCurrentScreen('home'); }} className="p-2 rounded-full hover:bg-gray-100">
      ...
      <div className="absolute bottom-0 w-full h-16 bg-white shadow-lg flex justify-around items-center">
        <NavButton icon={<lucide_react_1.AlertCircle size={24}/>} label="SOS" active={false} special={true}/>
        <NavButton icon={<lucide_react_1.Phone size={22}/>} label="Contacts" active={currentScreen === 'contacts'} onClick={function () { return setCurrentScreen('contacts'); }}/>
        <NavButton icon={<lucide_react_1.MapPin size={22}/>} label="Map" active={currentScreen === 'map'} onClick={function () { return setCurrentScreen('map'); }}/>
        <NavButton icon={<lucide_react_1.Home size={22}/>} label="Evac" active={currentScreen === 'evacuation'} onClick={function () { return setCurrentScreen('evacuation'); }}/>
        <NavButton icon={<lucide_react_1.Menu size={22}/>} label="More" active={menuOpen} onClick={function () { return setMenuOpen(!menuOpen); }}/>
      </div>
      
      {/* Menu Overlay */}
      {menuOpen && (<div className="absolute inset-0 bg-black bg-opacity-50" onClick={function () { return setMenuOpen(false); }}>
          <div className="absolute bottom-16 right-0 w-40 bg-white rounded-tl-lg shadow-lg p-2" onClick={function (e) { return e.stopPropagation(); }}>
            <MenuItem icon={<lucide_react_1.Heart size={18}/>} label="Medical" onClick={function () {
                setCurrentScreen('medical');
                setMenuOpen(false);
            }}/>
            <MenuItem icon={<lucide_react_1.FileText size={18}/>} label="Documents" onClick={function () {
                setCurrentScreen('documents');
                setMenuOpen(false);
            }}/>
            <MenuItem icon={<lucide_react_1.BookOpen size={18}/>} label="Guidelines" onClick={function () {
                setCurrentScreen('guidelines');
                setMenuOpen(false);
            }}/>
            <MenuItem icon={<lucide_react_1.User size={18}/>} label="Profile" onClick={function () {
...
function HomeScreen(_a) {
    ...
    return (<div className="h-full flex flex-col p-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <QuickTile icon={<lucide_react_1.Phone size={28}/>} label="Emergency Contacts" color="bg-blue-500" onClick={function () { return navigate('contacts'); }}/>
        <QuickTile icon={<lucide_react_1.MapPin size={28}/>} label="Emergency Locations" color="bg-green-500" onClick={function () { return navigate('map'); }}/>
        <QuickTile icon={<lucide_react_1.Home size={28}/>} label="Evacuation Points" color="bg-orange-500" onClick={function () { return navigate('evacuation'); }}/>
        <QuickTile icon={<lucide_react_1.Heart size={28}/>} label="Medical Info" color="bg-purple-500" onClick={function () { return navigate('medical'); }}/>
      </div>
    </div>);
}
...
function MapScreen() {
    return (<div className="h-full p-4 flex flex-col">
      <div className="bg-white rounded-lg shadow-sm p-4 mb-4">
      ...
      <div className="flex-1 bg-gray-200 rounded-lg overflow-hidden relative">
        <div className="absolute inset-0 flex items-center justify-center">
          <lucide_react_1.MapPin size={24} className="text-red-500"/>
        </div>
      ...
      <div className="flex-1 bg-gray-200 rounded-lg overflow-hidden relative">
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="flex flex-col items-center">
            <lucide_react_1.Home size={24} className="text-orange-500"/>
        ...
        <div className="absolute top-1/4 right-1/4">
          <div className="bg-orange-500 p-1 rounded-full">
            <lucide_react_1.Home size={16} className="text-white"/>
          </div>
        </div>
...
function NavButton(_a) {
    var icon = _a.icon, label = _a.label, active = _a.active, onClick = _a.onClick, special = _a.special;
    return (<button className={"flex flex-col items-center justify-center w-16 ".concat(active ? 'text-blue-500' : 'text-gray-600')} onClick={onClick}>
      <div className={"p-1 ".concat(special ? 'bg-red-600 text-white rounded-full' : '')}>
        {icon}
      </div>
...
function MenuItem(_a) {
    ...
    return (<button className="flex items-center w-full p-2 hover:bg-gray-100 rounded" onClick={onClick}>
      <span className="mr-2 text-gray-600">{icon}</span>
      <span>{label}</span>
    </button>);
}
function QuickTile(_a) {
    var icon = _a.icon, label = _a.label, color = _a.color, onClick = _a.onClick;
    return (<button className="bg-white rounded-xl shadow-sm overflow-hidden flex flex-col items-center p-4" onClick={onClick}>
      <div className={"".concat(color, " text-white p-3 rounded-full mb-2")}>
        {icon}
      </div>
...
function ContactItem(_a) {
    var name = _a.name, number = _a.number, relationship = _a.relationship;
    return (<div className="border-b border-gray-100 py-3 last:border-0 flex justify-between items-center">
      <div>
...
function InfoItem(_a) {
    var label = _a.label, value = _a.value, highlight = _a.highlight;
    return (<div className="border-b border-gray-100 py-2 last:border-0">
      <p className="text-sm text-gray-500">{label}</p>
      <p className={"font-medium ".concat(highlight ? 'text-red-600' : '')}>{value}</p>
    </div>);
}
function DocumentItem(_a) {
    ...
    return (<div className="border-b border-gray-100 py-3 last:border-0 flex justify-between items-center">
      <div className="flex items-center">
        <lucide_react_1.FileText size={20} className="text-gray-400 mr-2"/>
        <p className="font-medium">{title}</p>
      </div>
      <button className="text-blue-500">View</button>
    </div>);
}
function EvacuationItem(_a) {
    var title = _a.title, distance = _a.distance, type = _a.type;
    return (<div className="border-b border-gray-100 py-3 last:border-0 flex justify-between items-center">
      <div className="flex items-center">
        <div className="bg-orange-500 p-1 rounded-full mr-2">
          <lucide_react_1.Home size={16} className="text-white"/>
